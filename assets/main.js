(function() {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const shrinker = { active: false, offset: 0, factor: 0.7 };
    ...
    addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Shift' || e.key === 'ShiftLeft' || e.key === 'ShiftRight') {
        ...
        if (player.stamina > 0) {
          const actualCost = Math.min(player.stamina, haltCost);
          player.stamina = Math.max(0, player.stamina - actualCost);
          player.vx = 0;
          player.vy = 0;
    ...
    function updateJetpackParticles(dt) {
      ...
      for (let i = jetpackParticles.length - 1; i >= 0; i -= 1) {
        ...
        const lifeRatio = Math.min(1, p.life / p.lifespan);
        const fade = Math.pow(1 - lifeRatio, 1.1);
        p.s = p.initialSat * (0.4 + 0.6 * fade);
        p.o = p.initialOpacity * fade;
        p.w = p.initialW * (0.55 + 0.45 * fade);
        p.h = p.initialH * (0.45 + 0.55 * fade);

        if (lifeRatio >= 1 || p.o <= 0.02) {
    ...
    function applyShrink(active) {
      if (active === shrinker.active) return;
      if (active) {
        const oldSize = player.size;
        player.size = CONFIG.playerSize * shrinker.factor;
        shrinker.offset = (oldSize - player.size) / 2;
        player.y += shrinker.offset;
        shrinker.active = true;
      } else {
        player.y -= shrinker.offset;
        player.size = CONFIG.playerSize;
        shrinker.offset = 0;
        shrinker.active = false;
      }
    }

    function toggleShrink() {
      applyShrink(!shrinker.active);
    }
    ...
    function update(dt) {
      ...
      const usingJet = upIntent && player.stamina > 0;
      ...
      if (usingJet) {
        ...
        if (spawnCount > 0) {
          ...
          for (let i = 0; i < spawnCount; i += 1) {
            ...
            const lifespan = 2.3 + Math.random() * 1.3;
            const initialOpacity = 0.7;
            const initialSat = 100;
    ...
    function draw() {
      ...
      const topOfPlayer = worldToScreen(player.x, player.y - pr);
      ctx.save();
      ctx.font = '16px "Trebuchet MS", system-ui';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'bottom';
      ctx.fillStyle = 'rgba(255, 230, 140, 0.92)';
      ctx.fillText('hen', topOfPlayer.x, topOfPlayer.y - 5);
      ctx.restore();
    }
